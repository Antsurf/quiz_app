    ...
    allQuestionsAnswered() {
      ...
      return this.quiz.questions.every(q => this.answers[q.id]);
        ...
        averageScore() {
            ...
            const total = this.quizAttempts.reduce((sum, attempt) => {
                return sum + (attempt.score * 100);
            }, 0);
            
            return (total / this.quizAttempts.length).toFixed(1);
        },
        
        // Find maximum score - SIMPLE VERSION
        maxScore() {
            if (this.quizAttempts.length === 0) return 0;
            
            let max = 0;
            // Loop through each attempt
            for (let attempt of this.quizAttempts) {
                let percentage = attempt.score * 100;
                // If this percentage is higher than current max, update max
                if (percentage > max) {
                    max = percentage;
                }
            }
        ...
        // Find minimum score - SIMPLE VERSION
        minScore() {
            if (this.quizAttempts.length === 0) return 0;
            
            let min = 100; // Start with highest possible score
            // Loop through each attempt
            for (let attempt of this.quizAttempts) {
                let percentage = attempt.score * 100;
                // If this percentage is lower than current min, update min
                if (percentage < min) {
                    min = percentage;
                }
            }
